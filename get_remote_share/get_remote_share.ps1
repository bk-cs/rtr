...
$Out = foreach ($Value in (gci "Registry::\HKEY_USERS" -EA 0 | ? {
$_.Name -match 'S-\d-\d+-(\d+-){1,14}\d+$' }).PSChildName) {
    gci "Registry::\HKEY_USERS\$Value\Network" -EA 0 | % {
        [PSCustomObject] @{ Share = $_.PSChildName; RemotePath = $_.GetValue('RemotePath'); Sid = $Value;
            UserName = (gwmi Win32_UserAccount | ? { $_.SID -eq $Value }).Name }
    }
}
output $Out $Param "get_remote_share_$((Get-Date).ToFileTimeUtc()).json"