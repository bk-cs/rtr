...
$Sel = @('Name', 'DisplayName', 'DisplayGroup', 'Enabled', 'Profile', 'Direction', 'Action',
    'EdgeTraversalPolicy', 'LooseSourceMapping', 'LocalOnlyMapping', 'Owner', 'PrimaryStatus', 'EnforcementStatus',
    'PolicyStoreSource', 'PolicyStoreSourceType')
$Out = Get-NetFirewallRule -EA 0 | select $Sel | % {
    $_.PSObject.Properties | % {
        if ($_.Value -and $_.Value.ToString() -and $_.Value -isnot [array]) { $_.Value = $_.Value.ToString() }
    }
    if ($Param.Filter) {
        $_ | ? { $_.Name -match $Param.Filter -or $_.DisplayName -match $Param.Filter -or $_.DisplayGroup -match
            $Param.Filter }
    } else { $_ }
}
output $Out $Param "get_firewall_rule.ps1"