...
$Out = [PSCustomObject] @{ LastBootUpTime =
    (Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime.ToFileTimeUtc() }
output $Out $Param "get_last_boot_$((Get-Date).ToFileTimeUtc()).json"