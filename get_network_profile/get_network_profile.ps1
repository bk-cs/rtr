...
$Out = Get-NetConnectionProfile -EA 0 | select Name, InterfaceAlias, InterfaceIndex, NetworkCategory,
IPv4Connectivity, IPv6Connectivity | % {
    $_.PSObject.Properties | % { if ($_.Value.ToString()) { $_.Value = $_.Value.ToString() }}
    $_
}
output $Out $Param "get_network_profile_$((Get-Date).ToFileTimeUtc()).json"