...
$Sel = @('Name', 'InterfaceAlias', 'InterfaceIndex', 'NetworkCategory', 'IPv4Connectivity', 'IPv6Connectivity')
$Out = Get-NetConnectionProfile -EA 0 | select $Sel | % {
    $_.PSObject.Properties | % { if ($_.Value.ToString()) { $_.Value = $_.Value.ToString() }}
    $_
}
output $Out $Param "get_network_profile.ps1"