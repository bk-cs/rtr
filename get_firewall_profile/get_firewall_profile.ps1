...
$Sel = @('Name', 'Enabled', 'DefaultInboundAction', 'DefaultOutboundAction', 'AllowInboundRules',
    'AllowLocalFirewallRules', 'AllowLocalIPsecRules', 'AllowUserApps', 'AllowUserPorts',
    'AllowUnicastResponseToMulticast', 'NotifyOnListen', 'EnableStealthModeForIPSec', 'LogFileName',
    'LogMaxSizeKilobytes', 'LogAllowed', 'LogBlocked', 'LogIgnored', 'DisabledInterfaceAliases')
$Out = Get-NetFirewallProfile | select $Sel | % {
    $_.PSObject.Properties | % { if ($_.Value.ToString() -and $_.Name -ne 'DisabledInterfaceAliases') {
        $_.Value = $_.Value.ToString() }}
    $_
}
output $Out $Param "get_firewall_profile.ps1"