...
function parse ([object] $Default, [string] $JsonInput) {
    ...
    switch ($Param) {
        { -not $_.LogName } {
            throw "Missing required parameter 'LogName'."
        }
...
$Sel = @('TimeCreated','ProviderName','Id','Message')
$Out = try {
    Get-WinEvent -LogName $Param.LogName -MaxEvents 1000 | select $Sel | % {
...
output $Out $Param "get_eventlog.ps1"