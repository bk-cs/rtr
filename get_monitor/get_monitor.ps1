...
$Out = gwmi -Namespace root\wmi -Class WmiMonitorID -EA 0 | select ManufacturerName,
UserFriendlyName, SerialNumberID | % {
    $_.PSObject.Properties | ? { $_.Value -is [System.Array] } | % {
        $_.Value = ([System.Text.Encoding]::ASCII.GetString($_.Value -notmatch 0))
    }
    $_
}
output $Out $Param "get_monitor_$((Get-Date).ToFileTimeUtc()).json"