...
$Sel = @('TpmPresent', 'TpmReady', 'TpmEnabled', 'TpmActivated', 'TpmOwned', 'RestartPending',
    'ManufacturerId', 'ManufacturerIdTxt', 'ManufacturerVersion', 'ManagedAuthLevel', 'OwnerAuth',
    'OwnerClearDisabled', 'AutoProvisioning', 'LockedOut', 'LockoutHealTime', 'LockoutCount', 'LockoutMax',
    'SelfTest')
$Out = Get-Tpm -EA 0 | select $Sel
output $Out $Param "get_tpm.ps1"