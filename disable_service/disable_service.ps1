...
if (-not $Param.Name) { throw "Missing required parameter 'Name'." }
$Service = gsv | ? { $_.Name -eq $Param.Name }
if (-not $Service) { throw "No results for service '$($Param.Name)'." }
...
$Out = gsv -Name $Param.Name | select Name, Status, StartType
output $Out $Param "disable_service_$((Get-Date).ToFileTimeUtc()).json"