...
$Out = gwmi Win32_Service -EA 0 | select ProcessId, Name, PathName | % {
    if ($Param.Filter) { $_ | ? { $_.Name -match $Param.Filter -or $_.PathName -match $Param.Filter }}
    else { $_ }
}
output $Out $Param "get_service_$((Get-Date).ToFileTimeUtc()).json"