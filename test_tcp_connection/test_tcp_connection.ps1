...
$Out = Test-NetConnection -ComputerName $Param.Destination -Port $Param.Port | select ComputerName,
RemoteAddress, RemotePort, SourceAddress, InterfaceAlias, TcpTestSucceeded | % {
...
output $Out $Param "test_tcp_connection_$((Get-Date).ToFileTimeUtc()).json"