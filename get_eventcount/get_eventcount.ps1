...
$Out = Get-WinEvent -ListLog * | ? { $_.RecordCount } | select RecordCount, LogName | % {
    if ($Param.Filter) { $_ | ? { $_.LogName -match $Param.Filter }} else { $_ }
}
output $Out $Param "get_eventcount.ps1"